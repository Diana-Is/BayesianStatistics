...
Rcpp::List mcmc_hidden_2(int H, int L, int Q, NumericMatrix potts_info, NumericMatrix Theta_s, NumericMatrix Theta_0_s, IntegerMatrix Delta_s, arma::mat P, double d_s, int iter, int burn, double mu, double mu_0, double sigma, double sigma_0, double dd, double ff, double a_d, double b_d) {
  ...
  int M = 1;
  int E = (H*L)*0.05;
  double tau = 0.1; //decay parameter
  double phi = 0.1;
  double hastings = 0;
  ...
  NumericVector d_store(iter); //storage for mcmc samples
  NumericMatrix P_1(H, L);
  NumericMatrix P_2(H, L);
  NumericMatrix P_3(H, L);
  NumericMatrix P_4(H, L);
  NumericMatrix theta_store(iter, Q*(Q - 1)/2); //interaction parameters in the areas of interest(AOI)
  NumericMatrix theta_0_store(iter, Q*(Q - 1)/2); //interaction parameters of the background
...
timer.step("start");
  ...
  for(i = 0; i < iter; i++)
  {
    ...
    timer.step("10000iter"); 
    }
    else if(i==19999){
      timer.step("20000iter"); 
    }

    // Update d
    d_temp = rgamma(1, d*d/phi, phi/d)(0); //rgamma(n, shape, rate = 1, scale = 1/rate) n-number of observ
  ...
  long n_timer = 1000000000;
  NumericVector res(timer);
  for (int res_iter=0; res_iter<res.size(); res_iter++) {

        res[res_iter] = res[res_iter] / n_timer;
    }
  return Rcpp::List::create(Rcpp::Named("P_1") = P_1, Rcpp::Named("P_2") = P_2, Rcpp::Named("P_3") = P_3, Rcpp::Named("P_4") = P_4, Rcpp::Named("theta") = theta_store, Rcpp::Named("d") = d_store, Rcpp::Named("theta_0") = theta_0_store, Rcpp::Named("Delta_ppi") = Delta_ppi, Rcpp::Named("Delta_sum") = Delta_sum, Rcpp::Named("accept") = accept,Rcpp::Named("timer")=res);
}
