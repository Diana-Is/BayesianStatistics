...
data$race[which(data$race==6)]<-4
data$educat[which(data$educat==99)]<-1
data$educat[which(data$educat==8)]<-2
data$age_quit[is.na(data$age_quit)]<-100
data$height[which(data$pid==115174)]<-mean(data$height[which(data$gender==2)],na.rm=T)
data$height[which(data$pid==124606)]<-mean(data$height[which(data$gender==1)],na.rm=T)
data$weight[is.na(data$weight)]<-mean(data$weight[which(data$gender==1)],na.rm=T)
data$marital[which(data$marital==1)]<-0
data$marital[which(data$marital==2)]<-1
data$marital[which(data$marital==3)]<-0
data$marital[which(data$marital==4)]<-0
data$marital[which(data$marital==5)]<-0
data$marital[which(data$marital==9)]<-0
#--------------------------------------------------------------------------
...
data$smokework[is.na(data$smokework)]<-0
...
data$de_stag_7thed[which(data$de_stag_7thed==3)]<-1
data$de_stag_7thed[which(data$de_stag_7thed==4)]<-2
data$de_stag_7thed[which(data$de_stag_7thed==6)]<-3
data$de_stag_7thed[which(data$de_stag_7thed==7)]<-4
data$de_stag_7thed[which(data$de_stag_7thed==8)]<-5
data$de_stag_7thed[which(data$de_stag_7thed==9)]<-6
data$de_stag_7thed[which(data$de_stag_7thed==10)]<-7
#--------------------------------------------------------------------------
...
#--------------------------------------------------------------------------
data[c("dcfdeathlc","dcficd","deathcutoff","deathstat","finaldeathlc","hasdcf","ndicd")]<-list(NULL)
...
#data[c("diagasbe","diagadas","diagchas","diagbron","diagchro","diagcopd","diagdiab","diagemph","diagfibr","diaghear","diagpneu","diagsarc","diagsili","diagtube","diaghype","diagstro")]<-list(NULL)
#--------------------------------------------------------------------------
...
data$lss_alcohol_num[which(data$lss_alcohol_num==3)]<-4
data$lss_alcohol_num[which(data$lss_alcohol_num==4)]<-6
data$lss_alcohol_num[which(data$lss_alcohol_num==5)]<-8

#creating new aggregation variable 
data$lesionsize[is.na(data$lesionsize)]<-mean(data$lesionsize,na.rm = T)
data$alcohol_per_year[is.na(data$lss_alcohol_num)]<-0
#monthly or less often
data$alcohol_per_year[which(data$lss_alcohol_freq==2)]<-12*data$lss_alcohol_num[which(data$lss_alcohol_freq==2)]
#2 to 4 times a month
data$alcohol_per_year[which(data$lss_alcohol_freq==3)]<-36*data$lss_alcohol_num[which(data$lss_alcohol_freq==3)]
#2 or more times a week
data$alcohol_per_year[which(data$lss_alcohol_freq==4)]<-104*data$lss_alcohol_num[which(data$lss_alcohol_freq==4)]
#4 or more times a week
data$alcohol_per_year[which(data$lss_alcohol_freq==5)]<-208*data$lss_alcohol_num[which(data$lss_alcohol_freq==5)]
data$alcohol_per_year[is.na(data$alcohol_per_year)]<-12*1
data[c("lss_alcohol_freq","lss_alcohol_num")]<-list(NULL)
...
data<-data[which(data$pid!=107099),]
data<-data[which(data$pid!=100147),]



data$event[is.na(data$death_days)]<-0
data$event[is.na(data$event)]<-1


#-------------------------------------------------------------------
#Standartization of non-categorical variables
#-------------------------------------------------------------------
data$smokeage<-(data$smokeage-mean(data$smokeage))/sd(data$smokeage)
data$age_quit<-(data$age_quit-mean(data$age_quit))/sd(data$age_quit)
data$pkyr<-(data$pkyr-mean(data$pkyr))/sd(data$pkyr)
data$age<-(data$age-mean(data$age))/sd(data$age)
data$lesionsize<-(data$age-mean(data$lesionsize))/sd(data$lesionsize)
...
write.csv(data,"data_cleaned3.csv")
data <- read.csv("data_cleaned3.csv", header=T)
#aggregating cardio-vascular diagnose
data$CardVas[which(data$diaghear==1)]<-1
data$CardVas[which(data$diaghype==1)]<-1
data$CardVas[which(data$diagstro==1)]<-1
data$CardVas[is.na(data$CardVas)]<-0
#aggregating lung diseases diagnose
data$Lung_ill<-rep(0,161)
data$Lung_ill[which(data$diagasbe==1)]<-1
data$Lung_ill[which(data$diagadas==1)]<-1
data$Lung_ill[which(data$diagchas==1)]<-1
data$Lung_ill[which(data$diagbron==1)]<-1
data$Lung_ill[which(data$diagchro==1)]<-1
data$Lung_ill[which(data$diagcopd==1)]<-1
data$Lung_ill[which(data$diagemph==1)]<-1
data$Lung_ill[which(data$diagpneu==1)]<-1
data$Lung_ill[which(data$diagsarc==1)]<-1
data$Lung_ill[which(data$diagtube==1)]<-1
data[c("diagasbe","diagadas","diagchas","diagbron","diagchro","diagcopd","diagemph","diagfibr","diaghear","diagpneu","diagsarc","diagsili","diagtube","diaghype","diagstro")]<-list(NULL)

write.csv(data,"data_cleaned4.csv")
rm(data)
