...
myvars <- c("pid","death_days","diag_date","fup_days",
            "rndgroup","diagasbe","diagstro","de_stag_7thed","progression_num","lym.given.lym",
...
rndgroup<-t(matrix(data$rndgroup,nrow = 5,ncol = 296))[,1]
progression_num<-t(matrix(data$progression_num,nrow = 5,ncol = 296))[,1]
Asbestosis<-t(matrix(data$Asbestosis,nrow = 5,ncol = 296))[,1]
Stroke<-t(matrix(data$Stroke,nrow = 5,ncol = 296))[,1]
...
save(outputAFT,file='AFT_output_rand2.Rdata')   # we save the chain
